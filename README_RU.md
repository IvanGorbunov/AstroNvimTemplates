...
- [AstroNvim Python Development Environment](#astronvim-python-development-environment)
  ...
  - [🔎 Отображение скрытых файлов в Neo-tree](#-отображение-скрытых-файлов-в-neo-tree)
    - [Быстрое переключение (временно)](#быстрое-переключение-временно)
    - [Постоянная конфигурация](#постоянная-конфигурация)
    - [📌 Справочник горячих клавиш Neo-tree](#-справочник-горячих-клавиш-neo-tree)
...
## 🔎 Отображение скрытых файлов в Neo-tree

По умолчанию Neo-tree фильтрует скрытые файлы (`dotfiles`, `.venv`, `.gitignore` и т.п.).

### Быстрое переключение (временно)
В файловом дереве (`:Neotree`) нажмите:
```
zh
```
Это стандартный хоткей — **переключить скрытые файлы**.
...
После этого: перезапустите Neovim → `:Lazy sync` → откройте дерево (`<leader>e`).

### 📌 Справочник горячих клавиш Neo-tree
| Клавиша | Действие |
|---------|----------|
| `<leader>e` | Открыть/закрыть файловое дерево |
| `<leader>o` | Фокус на файловом дереве |
| `zh` | Переключить видимость скрытых файлов |
| `zf` | Переключить видимость gitignored файлов |
| `a` | Создать новый файл/папку |
| `d` | Удалить файл/папку |
| `r` | Переименовать файл/папку |
| `c` | Копировать файл/папку |
| `m` | Переместить файл/папку |
| `y` | Копировать путь к файлу |
| `Y` | Копировать относительный путь |
| `s` | Открыть в разделенном окне |
| `v` | Открыть в вертикальном разделении |
| `t` | Открыть в новой вкладке |
| `P` | Предварительный просмотр файла |
| `<` | Перейти на уровень выше |
| `.` | Установить как корневую директорию |
